...
if(isset($_POST['submit']))
{
    ...
    $sql = "SELECT * from new_customer where id=$from";
    $query = mysqli_query($conn,$sql);
    $sql1 = mysqli_fetch_array($query); // returns array or output of user from which the amount is to be transferred.

    $sql = "SELECT * from new_customer where id=$to";
    $query = mysqli_query($conn,$sql);
    ...
    if (($amount)<0)
   {
    ...
    // constraint to check zero values
         ...
         echo "</script>";
                ...
                $sql = "UPDATE new_customer set amount=$newbalance where id=$from";
	...
	<div class="container">
        <h2 class="text-center pt-4" style="color : black;">Transaction</h2>
            <?php
                ...
                $sid=$_GET['id'];
                $sql = "SELECT * FROM  new_customer where id=$sid";
